...
namespace Autohand {
    ...
    public class AutoHandPlayer : MonoBehaviour {
        ...
        public GlobalValue<bool> snapTurning;
        [Tooltip("turn speed when not using snap turning - if snap turning, represents angle per snap")]
        [ShowIf("snapTurning")]
        public GlobalValue<float> snapTurnAngle;
        [HideIf("snapTurning")]
        public GlobalValue<float> smoothTurnSpeed;
        ...
        protected virtual void UpdateTurn(float deltaTime) {
            ...
            //Snap turning
            if(snapTurning.Value) {
                if(Mathf.Abs(turningAxis) > turnDeadzone && axisReset) {
                    var angle = turningAxis > turnDeadzone ? snapTurnAngle.Value : -snapTurnAngle.Value;
                ...
                trackingContainer.RotateAround(headCamera.transform.position, Vector3.up, smoothTurnSpeed.Value * (turningAxis - turnDeadzone) * deltaTime);
